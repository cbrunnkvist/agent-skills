...
> [!CAUTION]
> All paths in the bundled spec are already prefixed with `/api` (e.g. `GET /api/endpoints`).
> Use them as-is against the Portainer server URL — do NOT add a second `/api` prefix.
