...
Everything in the Portainer UI is also doable over its HTTP API. This skill covers how to call it correctly.

**Two spec locations live side by side in this skill:**

1. `references/spec/` — chunk-based spec at a single EE version (generated by `make specs`). Fast for grep-driven endpoint lookup. The header in `INDEX.md` tells you which version.
2. `references/spec-versions/` — versioned full OpenAPI 3.0 YAML for CE and EE, across multiple release streams (LTS + STS). Use `INDEX.md` there to resolve the correct spec for the server version and edition you're talking to, then reference `TAXONOMY.md` for the version→file mapping and edition surface diff.

See *Finding the right endpoint* below.
...
**1. Check what you're talking to and resolve the correct spec.** `GET /api/system/version` returns the version; `GET /api/settings/public` tells you edition-relevant config without auth. Then consult `references/spec-versions/INDEX.md` → `TAXONOMY.md` to pick the right spec file for this server's version and edition.

If the server version doesn't have a dedicated file, use the bundled chunk spec (`references/spec/`) as a fallback — the header in `references/spec/INDEX.md` tells you which EE version it describes. On Community Edition, EE-only paths (GitOps, observability, policies, licenses, KaaS/cloud provisioning, edge update schedules) return 404 — a 404 there means "wrong edition", not "wrong URL". Compare the instance's version against the spec's: on a minor mismatch, treat the spec as approximate at the edges — recently added endpoints and fields are the suspect set when a documented call misbehaves (see *Version skew* below).
...
Two spec sources are available. Use the versioned full specs in `references/spec-versions/` when the server version or edition doesn't match the bundled chunk spec, or when you need CE definitions. Otherwise the chunk spec is faster for interactive lookup.

**Versioned full specs** (`references/spec-versions/`): each file is a complete OpenAPI 3.0 YAML with every path, schema, and parameter. Grep directly:

```bash
# Find an endpoint in the matching version+edition spec:
grep -n 'stacks/create' references/spec-versions/ee-2.39-lts.yaml
```

**Chunk spec** (`references/spec/`): one self-contained file per API area, so a `$ref` never points outside the file you're reading. Two generated entry points sit alongside:
